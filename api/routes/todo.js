...
router.route('/todos')
  .all(isAuthenticated)
  .get((req, res) => {
    getRepository(ToDo).find({ where: {user: { id: req.user.id} } }).then((todos) => {
      res.send(todos);
    });
  })
  .post((req, res) => {
...
router.route('/categories')
  .all(isAuthenticated)
  .get((req, res) => {
    getRepository(Category).find({ where: { user: { id: req.user.id} } }).then((categories) => {
      res.send(categories);
    });
  });
router.route('/todos/:id')
  ...
  .all((req, res, next) => {
    getRepository(ToDo).findOneOrFail(
      { where: { user: {id: req.user.id}, id: req.params.id } },
    ).then((_foundTodo) => {
      req.todo = _foundTodo;
      next();
    }, () => {
